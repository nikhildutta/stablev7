/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AddressWhereUniqueInput } from "../../address/base/AddressWhereUniqueInput";
import { ValidateNested, IsOptional, IsString, IsDate } from "class-validator";
import { Type } from "class-transformer";
import { ContactHashtagsMapCreateNestedManyWithoutContactDetailsInput } from "./ContactHashtagsMapCreateNestedManyWithoutContactDetailsInput";

@InputType()
class ContactDetailCreateInput {
  @ApiProperty({
    required: true,
    type: () => AddressWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => AddressWhereUniqueInput)
  @Field(() => AddressWhereUniqueInput)
  addresses!: AddressWhereUniqueInput;

  @ApiProperty({
    required: false,
    type: () => ContactHashtagsMapCreateNestedManyWithoutContactDetailsInput,
  })
  @ValidateNested()
  @Type(() => ContactHashtagsMapCreateNestedManyWithoutContactDetailsInput)
  @IsOptional()
  @Field(() => ContactHashtagsMapCreateNestedManyWithoutContactDetailsInput, {
    nullable: true,
  })
  contactHashtagsMap?: ContactHashtagsMapCreateNestedManyWithoutContactDetailsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  contactName?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  contactNumber?: string | null;

  @ApiProperty({
    required: false,
  })
  @IsDate()
  @Type(() => Date)
  @IsOptional()
  @Field(() => Date, {
    nullable: true,
  })
  updatedAt?: Date | null;
}

export { ContactDetailCreateInput as ContactDetailCreateInput };
