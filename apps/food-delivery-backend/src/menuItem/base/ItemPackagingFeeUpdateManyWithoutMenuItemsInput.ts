/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ItemPackagingFeeWhereUniqueInput } from "../../itemPackagingFee/base/ItemPackagingFeeWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class ItemPackagingFeeUpdateManyWithoutMenuItemsInput {
  @Field(() => [ItemPackagingFeeWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ItemPackagingFeeWhereUniqueInput],
  })
  connect?: Array<ItemPackagingFeeWhereUniqueInput>;

  @Field(() => [ItemPackagingFeeWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ItemPackagingFeeWhereUniqueInput],
  })
  disconnect?: Array<ItemPackagingFeeWhereUniqueInput>;

  @Field(() => [ItemPackagingFeeWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ItemPackagingFeeWhereUniqueInput],
  })
  set?: Array<ItemPackagingFeeWhereUniqueInput>;
}

export { ItemPackagingFeeUpdateManyWithoutMenuItemsInput as ItemPackagingFeeUpdateManyWithoutMenuItemsInput };
