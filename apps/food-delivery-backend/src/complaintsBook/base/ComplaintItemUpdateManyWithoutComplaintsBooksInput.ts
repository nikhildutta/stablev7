/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ComplaintItemWhereUniqueInput } from "../../complaintItem/base/ComplaintItemWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class ComplaintItemUpdateManyWithoutComplaintsBooksInput {
  @Field(() => [ComplaintItemWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ComplaintItemWhereUniqueInput],
  })
  connect?: Array<ComplaintItemWhereUniqueInput>;

  @Field(() => [ComplaintItemWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ComplaintItemWhereUniqueInput],
  })
  disconnect?: Array<ComplaintItemWhereUniqueInput>;

  @Field(() => [ComplaintItemWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ComplaintItemWhereUniqueInput],
  })
  set?: Array<ComplaintItemWhereUniqueInput>;
}

export { ComplaintItemUpdateManyWithoutComplaintsBooksInput as ComplaintItemUpdateManyWithoutComplaintsBooksInput };
