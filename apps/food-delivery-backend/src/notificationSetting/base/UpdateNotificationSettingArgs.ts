/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { NotificationSettingWhereUniqueInput } from "./NotificationSettingWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { NotificationSettingUpdateInput } from "./NotificationSettingUpdateInput";

@ArgsType()
class UpdateNotificationSettingArgs {
  @ApiProperty({
    required: true,
    type: () => NotificationSettingWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => NotificationSettingWhereUniqueInput)
  @Field(() => NotificationSettingWhereUniqueInput, { nullable: false })
  where!: NotificationSettingWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => NotificationSettingUpdateInput,
  })
  @ValidateNested()
  @Type(() => NotificationSettingUpdateInput)
  @Field(() => NotificationSettingUpdateInput, { nullable: false })
  data!: NotificationSettingUpdateInput;
}

export { UpdateNotificationSettingArgs as UpdateNotificationSettingArgs };
